...
uint8_t numberLoop(int8_t value, uint8_t loopLength) {
  if (value < 0) {
    value += loopLength;
  } else if (value > loopLength)
    value -= loopLength;
  return uint8_t(value);
}
int8_t cycleVector(int8_t position, int8_t target, uint8_t cycleLength) {  //polar vectors
